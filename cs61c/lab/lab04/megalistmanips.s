...
mapLoop:
    # error3: the address of the array should be 0(s0) not (add t1, s0, x0) 
    lw t1, 0(s0)      # load the address of the array of current node into t1
    lw t2, 4(s0)        # load the size of the node's array into t2
    # error5 offset should multiply the size of 4
    slli t6, t0, 2
    add t1, t1, t6      # offset the array address by the count
    lw a0, 0(t1)        # load the value at that address into a0
    # error4 not save t1 before calling func
    addi sp, sp, -4
    sw t1, 0(sp)
    ...
    # error1 la a0, 8(s0) should be lw a0, 8(s0)
    lw a0, 8(s0)        # load the address of the next node into a0
    # error2 la a1, 8(s1) should be add a1, s1, x0
